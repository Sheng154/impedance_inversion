...
from thin_bed import imps, mback, model_trace, wavelet, low_filtered_imp, high_filtered_imp, omtx
...
toolbox.register("prior_range", np.random.normal, 0, 150)
...
toolbox.register("mutate", tools.mutGaussian, mu=0, sigma=20, indpb=0.05)
...
while Error > 2 and g < generations:
    ...
    for ind in offspring_whole:
        ...
        for k in range(len(ind)):
            diff_imp.append(abs((ind[k] - imps[k]) / imps[k]))
...
plt.show()
B = erroreval(best_ind_whole_lp, imps)
C = erroreval(best_ind_whole_lp1, imps)
print(B)
print(C)
